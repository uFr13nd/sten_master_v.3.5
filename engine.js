/* STEN_MASTER v3.5 — pure calculation engine.
...
(function (root, factory) {
  ...
  function calcWall(category, mat, finishType, inputs, model) {
    ...
    // Exact logic of the v3.5 main sheet.
    ...
    const total = costObj + deliveryCost + finishCost;
  ...
  function calcScenario(scenario, project, inputs, model) {
    ...
    for (const category of ['ext', 'inter', 'part']) {
      ...
      const sameMaterial = Boolean(base.material) && selected.material === base.material;
      if (!base.material || sameMaterial) {
        changedByCategory[category] = 0;
        continue;
      }
      ...
      changedTotal += cats[category].total;
    }
